...
\paragraph{}The different panels created with iceLogo can be saved to a \textit{.pdf} file. A save selection panel, where specific visualization panels can be selected, is opened after that the \textit{save} button \includegraphics[height=5ex]{save.png} is clicked. Furthermore, the precise data model which is being used by the graphics can also be saved as a .CSV file by enabling the checkbox in the save selection panel.
